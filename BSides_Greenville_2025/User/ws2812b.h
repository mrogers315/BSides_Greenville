...
void WS2812B_Hue_Table(uint8_t num_leds, uint8_t hue_index, uint8_t brightness){
    for(int i=0; i<num_leds; i++){
        WS2812B_SendColor(hue_r[((hue_index)+i*16)%256]/brightness, hue_g[((hue_index)+i*16)%256]/brightness, hue_b[((hue_index)+i*16)%256]/brightness);
    }
    Delay_Ms(50);
}
void WS2812B_FadeColors(uint8_t num_leds, uint8_t brightness) {
    ...
    int step = 5; // Adjust step size for smoother transition
        ...
        {
            for(int i=0; i<num_leds; i++){            
                WS2812B_SendColor(red/brightness, 0, blue/brightness);
            }
        ...
        {
            for(int i=0; i<num_leds; i++){
                WS2812B_SendColor(0, green/brightness, blue/brightness);
            }
            Delay_Ms(50);
        }
        ...
        {
            for(int i=0; i<num_leds; i++){
                WS2812B_SendColor(red/brightness, green/brightness, 0);
            }
            Delay_Ms(50);
        }
}

void WS2812B_ChaseColor(uint8_t num_leds, uint8_t num_loops, uint8_t brightness)
{
    for(int k = 0; k < num_loops; k++)
    {
        for(int j = 0; j<16; j++)
        {
                for(int i=0; i<num_leds; i++){ 
                    if((i+j%4)%2){           
                        WS2812B_SendColor(0, 0, 255/brightness);
                    }else{
                        WS2812B_SendColor(0, 0, 0);
                    }
                }
                Delay_Ms(200); // Adjust delay for speed
        }
        for(int j = 0; j<16; j++)
        {
                for(int i=0; i<num_leds; i++){
                    if((i+j%4)%2){
                        WS2812B_SendColor(0, 255/brightness, 0);
                    }else{
                        WS2812B_SendColor(0, 0, 0);
                    }
                }
        ...
        for(int j = 0; j<16; j++)
        {
            // Fade from Green to Red
                ...
                for(int i=0; i<num_leds; i++){
                    if((i+j%4)%2){
                        WS2812B_SendColor(255/brightness, 0, 0);
                    }else{
                        WS2812B_SendColor(0, 0, 0);
                    }
                }
